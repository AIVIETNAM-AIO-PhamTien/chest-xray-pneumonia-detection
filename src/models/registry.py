...
def register_model(name: str) -> Callable[[ModelBuilder], ModelBuilder]:
    ...
    Args:
        name: Key usable as ``model.name`` in a YAML config.
...
def build_model(
    ...
    **params: object,
    ...
    Args:
        name: Registered model name (e.g. "resnet18", "simple_cnn").
    ...
    if name not in MODEL_REGISTRY:
        raise ValueError(
            f"Unsupported model: {name!r}. Available: {sorted(MODEL_REGISTRY)}"
        )
